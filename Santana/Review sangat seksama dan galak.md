...
[ \min_{\boldsymbol{\theta}} \langle\psi| \hat U^\dagger(\boldsymbol{\theta}) \hat H \hat U(\boldsymbol{\theta}) |\psi\rangle, ]  
...
## [ \widehat E_S(\theta)  
\frac{1}{N_s} \sum_{i=1}^{N_s} H\bigl(\Gamma(q_i,p_i\mid\theta)\bigr) ]  
...
## [ O(q,p)  
A(q,\theta) + B(q,\theta)\xi + C(q,\theta)\xi^2. ]  
Ekspektasinya terhadap (\xi) adalah:  
## [ \mathbb E_\xi[O]  
A + B\mathbb E[\xi] + C\mathbb E[\xi^2]. ]  
Karena manuskrip menetapkan:  
[ \mathbb E[\xi]=0,\qquad \mathbb E[\xi^2]=\hbar^2, ]  
maka:  
## [ \mathbb E_\xi[O]  
A+C\hbar^2. ]  
...
## [ \rho'(q')  
\rho!\left(A^{-1}(q'-d)\right) \left|\det A^{-1}\right|. ]  
...
* effective sample size;  
[ N_{\rm eff}\approx \frac{N_s}{2\tau_{\rm int}}; ]  
* acceptance rate;  
* trace plot;  
...
[ \operatorname{Var}(\overline O) \approx \frac{\sigma_O^2}{N_s} \left[ 1+2\sum_{k=1}^{\infty}\rho_O(k) \right]. ]  
...
dan:  
## [ T_{\rm ERMC,total}  
T_{\rm sample} + \sum_{\alpha}T_{\rm transform}(\alpha). ]  
...
Atau efficiency:  
## [ \mathcal E  
\frac{1} {\operatorname{Var}(\widehat E),T}. ]  
...
\hat p^2 +\lambda(\hat p\hat q^2+\hat q^2\hat p) +\lambda^2\hat q^4. ]  
...
[ q\mapsto f(q), \qquad p\mapsto g(q)+c_0p, ]  
